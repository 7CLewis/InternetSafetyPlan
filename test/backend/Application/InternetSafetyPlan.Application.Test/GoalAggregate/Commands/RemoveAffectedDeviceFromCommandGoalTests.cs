...
public class RemoveAffectedDeviceFromGoalCommandTests
{
    [Fact]
    public async Task RemoveAffectedDeviceFromGoal_ValidAffectedDevice_ReturnsSuccessfulResult()
    {
        // Arrange
        ...
        goal.AddAffectedDevice(device);

        context.Set<Goal>().Add(goal);

        await context.SaveChangesAsync();

        var command = new RemoveAffectedDeviceFromGoalCommand(fakeGoalId, fakeDeviceId);
        var handler = new RemoveAffectedDeviceFromGoalCommandHandler(context);
    ...
    [Fact]
    public async Task RemoveAffectedDeviceFromGoal_InvalidAffectedDevice_ReturnsFailureWithErrorMessage()
    {
        // Arrange
        var random = new Faker().Random;
        var fakeUserId = random.Guid();
        ...
        var nonexistentAffectedDeviceId = random.Guid();
        var command = new RemoveAffectedDeviceFromGoalCommand(fakeGoalId, nonexistentAffectedDeviceId);
        var handler = new RemoveAffectedDeviceFromGoalCommandHandler(context);
        ...
        result.Error.Should().Be(SharedApplicationErrors.NotFound(nameof(Device), nonexistentAffectedDeviceId));
    }
}
