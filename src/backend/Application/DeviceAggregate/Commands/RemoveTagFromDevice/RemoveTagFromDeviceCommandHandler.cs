...
// TODO: All Domain functionality in repository? Or leave these things in model? Probs repository (maybe?)
public class RemoveTagFromDeviceCommandHandler : ICommandHandler<RemoveTagFromDeviceCommand, Unit>
{
    private readonly IDatabaseContext _databaseContext;

    public RemoveTagFromDeviceCommandHandler(IDatabaseContext databaseContext)
    {
        _databaseContext = databaseContext;
    }

    public async Task<Result<Unit>> Handle(RemoveTagFromDeviceCommand request, CancellationToken cancellationToken)
    {
        try
        {
            var tag =
            ...
            var result = deviceWithTags.RemoveTag(tag);
